...
async function testMicroCMS() {
  ...
  console.log(
    ...
    process.env.MICROCMS_API_KEY
      ? process.env.MICROCMS_API_KEY.substring(0, 10) + "..."
      : "Not set",
  );
  console.log(
    "API Key length:",
    process.env.MICROCMS_API_KEY ? process.env.MICROCMS_API_KEY.length : 0,
  );
  console.log(
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `Data count: ${data.contents ? data.contents.length : "N/A"}`,
          );
          if (data.contents && data.contents.length > 0) {
            console.log(
              `First item title: ${data.contents[0].title || "No title"}`,
            );
          }
          return; // 成功したら終了
        } else {
